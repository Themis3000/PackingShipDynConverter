...
for page in pages:
    ...
    with open(f"./out/slips/{order_data['buyer_name']}_{order_data['order_number']}.html", "w", encoding="utf-16") as f:
        f.write(template_values(html_out, {"body": slip_html}))
    with open(f"./out/labels/{order_data['buyer_name']}_{order_data['order_number']}.html", "w", encoding="utf-16") as f:
